<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function togglePaymentMethodSection() {
            ...
            const selectedBookingType = document.querySelector('input[name="booking_type"]:checked')?.value || 'full_payment';
            const depositAmount = totalAmount * 0.5;

            // Determine the amount to display based on booking type
            const displayAmount = selectedBookingType === 'partial_booking' ? depositAmount : totalAmount;

            if (useBalanceCheckbox && useBalanceCheckbox.checked) {
                if (userBalance >= displayAmount) {
                    ...
                    if (selectedBookingType === 'partial_booking') {
                        paymentButton.innerHTML = '<i class="fas fa-wallet mr-2"></i>Book Now (50% Deposit) Using Balance';
                    } else {
                    ...
                    const remainingAmount = displayAmount - userBalance;
                    if (selectedBookingType === 'partial_booking') {
                        paymentButton.innerHTML = '<i class="fas fa-credit-card mr-2"></i>Book Now (50% Deposit) - <?php echo formatCurrency($total * 0.5 - $user['balance']); ?>';
                    } else {
                        paymentButton.innerHTML = '<i class="fas fa-credit-card mr-2"></i>Complete Purchase - <?php echo formatCurrency($total - $user['balance']); ?>';
                    }
                ...
                togglePaymentDetails();
                if (selectedBookingType === 'partial_booking') {
                    paymentButton.innerHTML = '<i class="fas fa-lock mr-2"></i>Book Now (50% Deposit) - <?php echo formatCurrency($total * 0.5); ?>';
                } else {
                    paymentButton.innerHTML = '<i class="fas fa-lock mr-2"></i>Complete Secure Purchase - <?php echo formatCurrency($total); ?>';
                }
        ...
        // Booking type change listener
        const bookingTypeRadios = document.querySelectorAll('input[name="booking_type"]');
        bookingTypeRadios.forEach(radio => {
            radio.addEventListener('change', function () {
                console.log('Booking type changed in checkout-js:', this.value);
    ...
    function showNotification(message, type = 'info') {
        ...
        notification.className = `fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg max-w-sm ${type === 'success' ? 'bg-green-100 text-green-800 border border-green-200' :
