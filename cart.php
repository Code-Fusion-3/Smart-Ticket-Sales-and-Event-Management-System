...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_quantity'])) {
        ...
        if ($quantity <= 0) {
            ...
            $checkSql = "SELECT ci.*, tt.available_tickets, tt.name as ticket_name
                        FROM cart_items ci
            ...
            if ($cartItem) {
                ...
                if ($quantity > $availableTickets) {
                    $_SESSION['error_message'] = "Sorry, only $availableTickets tickets available for " . htmlspecialchars($cartItem['ticket_name']);
                } else {
                    // Update quantity
                    $db->query("UPDATE cart_items SET quantity = $quantity, updated_at = NOW() WHERE id = $itemId AND cart_id = $cartId");
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-6">
        <i
            class="fas fa-exclamation-triangle mr-2"></i><?php echo $_SESSION['warning_message']; unset($_SESSION['warning_message']); ?>
    </div>
    <?php endif; ?>
