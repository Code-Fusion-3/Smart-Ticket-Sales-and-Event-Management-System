...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['complete_booking'])) {
    ...
    if ($booking && $booking['status'] === 'pending') {
        ...
        if ($user && $user['balance'] >= $remainingAmount) {
            try {
            ...
            if (!$user) {
                ...
                $_SESSION['error_message'] = "Insufficient balance. You need " . formatCurrency($remainingAmount) . " to complete your booking. Your current balance: " . formatCurrency($user['balance']);
            }
        }
    } else {
                                ...
                                <div class="mt-4 md:mt-0 md:ml-6 flex flex-col items-end">
                                    ...
                                    <?php if ($booking['status'] === 'pending'): ?>
                                        ...
                                        <p class="text-xs text-gray-500 mt-2 text-center">
                                            Pay remaining
                                            <?php echo formatCurrency($booking['total_amount'] - $booking['amount_paid']); ?>
                                        </p>
                                    <?php elseif ($booking['status'] === 'confirmed'): ?>
                                        <a href="my-tickets.php"
